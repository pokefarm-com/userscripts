...
class PFQAPI {
	 ...
	 * 
	 * Accepts a comma-separated list of Pokémon shortlinks (adopts) and returns the user shortlink and display name for each.
	 * The response preserves the order of the input. If a shortlink is invalid or not found, an error is returned for that entry.
	 * 
	 * @param {string} adopts Comma-separated list of Pokémon shortlinks (between 1 and 30)
	 * @returns {Promise<{
	 *  adopts: Array<{
	 *    shortlink: string,
	...
	async shelterDonors(adopts) {
		return (await PFQAPI.#request(`/pokemon/shelter/donors?adopts=${adopts}`)).adopts;
	}
	// #endregion
