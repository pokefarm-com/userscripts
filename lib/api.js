...
class PFQAPI {
	 ...
	 */
	async chains() {
		return (await PFQAPI.#request("/user/saved-chains")).chains;
	}
	...
	async #fetchTyperaceRotation() {
		...
		const key = `${$.USERID}.api.typeracerotation`;
	...
	/**
	 * Get the user's Type Race clicklist.
	 * 
	 * Returns a list of user shortlinks that the current user has clicked on during Type Race.
	 * 
	 * @param {number} [page=1] The page number to fetch.
	 * @returns {Promise<{users:string[],page:number}>}
	 */
	async #fetchTyperaceClicklist(page) {
		// get from localstorage if possible
		const key = `${$.USERID}.api.typeraceclicklist-${page}`;
		if (key in localStorage) {
			const cached = JSON.parse(localStorage[key]);
			if (cached.timestamp > Date.now()) {
				return { timestamp: cached.timestamp, users: cached.users, page: cached.page };
			}
		}
		const response = await PFQAPI.#request(`/user/typerace/clicklist?page=${page}`);
		// cache for 30 minutes
		const expires = Date.now() + 30 * 60 * 1000;
		localStorage[key] = JSON.stringify({ timestamp: expires, users: response.users, page: response.page });
		return { timestamp: expires, users: response.users, page: response.page };
	}
	 ...
	 * @returns {Promise<{users:string[],page:number}>}
	 */
	async typeraceClicklist(page=1) {
		return await this.#fetchTyperaceClicklist(page);
	}
	// #endregion
	...
	/**
	 * Get user info for a list of adopted Pokémon shortlinks.
	 * 
	 * Accepts an array of Pokémon shortlinks (adopts) and returns an object where keys are shortlinks and values are user info objects.
	 * The response preserves the order of the input. If a shortlink is invalid or not found, an error is returned for that entry.
	 * 
	 * @param {string[]} adopts Array of Pokémon shortlinks (between 1 and 30)
	 * @returns {Promise<{
	 *  [shortlink: string]: {
	 *    shortlink: string,
	 *    user_shortlink?: string,
	 *    user_name?: string,
	 *    error?: string
	 *  }
	 * }>}
	 */
	async shelterDonors(adopts) {
		if (!Array.isArray(adopts) || adopts.length < 1 || adopts.length > 30) {
			throw new Error("Adopts array must have between 1 and 30 items.");
		}
		const adoptsParam = adopts.join(',');
		const response = await PFQAPI.#request(`/pokemon/shelter/donors?adopts=${adoptsParam}`);
		
		const result = {};
		for (const item of response.adopts) {
			result[item.shortlink] = item;
		}
		return result;
	}
	// #endregion
